#include "voxel_instancer_blocky.h"
...
class VoxelInstancerBlockyRigidBody : public RigidBody {
	GDCLASS(VoxelInstancerBlockyRigidBody, RigidBody);

public:
	VoxelInstancerBlockyRigidBody() {
	...
	void attach(VoxelInstancerBlocky *parent) {
		_parent = parent;
	}
...
private:
	VoxelInstancerBlocky *_parent = nullptr;
...
VoxelInstancerBlocky::VoxelInstancerBlocky() {
...
void VoxelInstancerBlocky::clear_blocks() {
	VOXEL_PROFILE_SCOPE();
	// Destroy blocks, keep configured layers
	for (auto it = _blocks.begin(); it != _blocks.end(); ++it) {
		Block *block = *it;
		for (int i = 0; i < block->bodies.size(); ++i) {
			VoxelInstancerBlockyRigidBody *body = block->bodies[i];
...
void VoxelInstancerBlocky::clear_blocks_in_layer(int layer_id) {
...
void VoxelInstancerBlocky::clear_layers() {
	clear_blocks();
	for (unsigned int lod_index = 0; lod_index < _lods.size(); ++lod_index) {
		Lod &lod = _lods[lod_index];
		lod.layers.clear();
...
void VoxelInstancerBlocky::_notification(int p_what) {
	switch (p_what) {
		...
		case NOTIFICATION_PARENTED:
			_parent = Object::cast_to<VoxelTerrain>(get_parent());
			if (_parent != nullptr) {
		...
		case NOTIFICATION_TRANSFORM_CHANGED: {
			VOXEL_PROFILE_SCOPE_NAMED("VoxelInstancerBlocky::NOTIFICATION_TRANSFORM_CHANGED");
...
VoxelInstancerBlocky::Layer *VoxelInstancerBlocky::get_layer(int id) {
	Layer *ptr = _layers.getptr(id);
	CRASH_COND(ptr == nullptr);
	return ptr;
}

const VoxelInstancerBlocky::Layer *VoxelInstancerBlocky::get_layer_const(int id) const {
	const Layer *ptr = _layers.getptr(id);
	CRASH_COND(ptr == nullptr);
	return ptr;
}

void VoxelInstancerBlocky::process_mesh_lods() {
	VOXEL_PROFILE_SCOPE();
...
void VoxelInstancerBlocky::update_visibility() {
	if (!is_inside_tree()) {
		return;
	}
...
void VoxelInstancerBlocky::set_world(World *world) {
	for (auto it = _blocks.begin(); it != _blocks.end(); ++it) {
		Block *block = *it;
		if (block->multimesh_instance.is_valid()) {
...
void VoxelInstancerBlocky::set_up_mode(UpMode mode) {
...
VoxelInstancerBlocky::UpMode VoxelInstancerBlocky::get_up_mode() const {
	return _up_mode;
}

void VoxelInstancerBlocky::set_library(Ref<VoxelInstanceLibrary> library) {
	if (library == _library) {
		return;
	}
...
Ref<VoxelInstanceLibrary> VoxelInstancerBlocky::get_library() const {
	return _library;
}

void VoxelInstancerBlocky::regenerate_layer(uint16_t layer_id, bool regenerate_blocks) {
	VOXEL_PROFILE_SCOPE();
	ERR_FAIL_COND(_parent == nullptr);
...
void VoxelInstancerBlocky::update_layer_meshes(int layer_id) {
	Ref<VoxelInstanceLibraryItem> item = _library->get_item(layer_id);
	ERR_FAIL_COND(item.is_null());
...
void VoxelInstancerBlocky::on_library_item_changed(int item_id, VoxelInstanceLibraryItem::ChangeType change) {
	ERR_FAIL_COND(_library.is_null());
...
void VoxelInstancerBlocky::add_layer(int layer_id, int lod_index) {
#ifdef DEBUG_ENABLED
...
void VoxelInstancerBlocky::remove_layer(int layer_id) {
	Layer *layer = get_layer(layer_id);
...
void VoxelInstancerBlocky::remove_block(unsigned int block_index) {
#ifdef DEBUG_ENABLED
...
void VoxelInstancerBlocky::on_data_block_loaded(Vector3i grid_position, unsigned int lod_index,
...
void VoxelInstancerBlocky::on_mesh_block_enter(Vector3i render_grid_position, unsigned int lod_index, Array surface_arrays) {
...
void VoxelInstancerBlocky::on_mesh_block_exit(Vector3i render_grid_position, unsigned int lod_index) {
	if (lod_index >= _lods.size()) {
		return;
	}
...
void VoxelInstancerBlocky::save_all_modified_blocks() {
	for (unsigned int lod_index = 0; lod_index < _lods.size(); ++lod_index) {
		Lod &lod = _lods[lod_index];
...
int VoxelInstancerBlocky::create_block(Layer *layer, uint16_t layer_id, Vector3i grid_position) {
...
void VoxelInstancerBlocky::update_block_from_transforms(int block_index, ArraySlice<const Transform> transforms,
	...
	if (collision_shapes.size() > 0) {
		...
		for (int instance_index = transforms.size(); instance_index < block->bodies.size(); ++instance_index) {
			VoxelInstancerBlockyRigidBody *body = block->bodies[instance_index];
			body->detach_and_destroy();
		}
...
void VoxelInstancerBlocky::create_render_blocks(Vector3i render_grid_position, int lod_index, Array surface_arrays) {
	VOXEL_PROFILE_SCOPE();
	ERR_FAIL_COND(_library.is_null());
...
void VoxelInstancerBlocky::save_block(Vector3i data_grid_pos, int lod_index) const {
	VOXEL_PROFILE_SCOPE();
	ERR_FAIL_COND(_library.is_null());
...
void VoxelInstancerBlocky::on_area_edited(Rect3i p_voxel_box) {
	VOXEL_PROFILE_SCOPE();
	ERR_FAIL_COND(_parent == nullptr);
			...
			render_blocks_box.for_each_cell([layer, &blocks, voxel_tool, p_voxel_box, parent_transform, block_size_po2, &lod, data_blocks_box](
				...
				for (int instance_index = 0; instance_index < instance_count; ++instance_index) {
					...
					if (block->bodies.size() > 0) {
						VoxelInstancerBlockyRigidBody *rb = block->bodies[instance_index];
						// Detach so it won't try to update our instances, we already do it here
						rb->detach_and_destroy();

						VoxelInstancerBlockyRigidBody *moved_rb = block->bodies[last_instance_index];
						if (moved_rb != rb) {
...
void VoxelInstancerBlocky::on_body_removed(Vector3i data_block_position, unsigned int render_block_index, int instance_index) {
	...
	VoxelInstancerBlockyRigidBody *moved_body = block->bodies[last_instance_index];
...
int VoxelInstancerBlocky::debug_get_block_count() const {
	return _blocks.size();
}

Dictionary VoxelInstancerBlocky::debug_get_instance_counts() const {
	Dictionary d;
...
String VoxelInstancerBlocky::get_configuration_warning() const {
	if (_parent == nullptr) {
		return TTR("This node must be child of a VoxelTerrain.");
	}
	if (_library.is_null()) {
...
void VoxelInstancerBlocky::_bind_methods() {
	ClassDB::bind_method(D_METHOD("set_library", "library"), &VoxelInstancerBlocky::set_library);
	ClassDB::bind_method(D_METHOD("get_library"), &VoxelInstancerBlocky::get_library);

	ClassDB::bind_method(D_METHOD("set_up_mode", "mode"), &VoxelInstancerBlocky::set_up_mode);
	ClassDB::bind_method(D_METHOD("get_up_mode"), &VoxelInstancerBlocky::get_up_mode);

	ClassDB::bind_method(D_METHOD("debug_get_block_count"), &VoxelInstancerBlocky::debug_get_block_count);
	ClassDB::bind_method(D_METHOD("debug_get_instance_counts"), &VoxelInstancerBlocky::debug_get_instance_counts);
